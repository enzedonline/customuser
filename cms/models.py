...
class ThemePage(TranslatablePage):
    theme = models.ForeignKey(
        Theme, on_delete=models.SET_NULL, null=True, related_name="themepages"
    )
    intro = RichTextField(blank=True)
    ...
    def articlepages(self):
        return (
            self.theme.articlepages.filter(language=self.language)
            .live()
...
class ArticlePage(TranslatablePage):
    ...
    themes = ParentalManyToManyField(
        Theme, blank=True, related_name="articlepages", verbose_name=_("Themes")
    )
    ...
    def themepages(self):
        return ThemePage.objects.filter(
            theme__in=self.themes.all(), language=self.language
        )
...
class MenuItem(Orderable):
    ...
    def trans_page(self, language_code):
        if self.link_page:
            # can_page = (
            #     self.link_page.canonical_page
            #     if self.link_page.canonical_page
            #     else self.link_page
            # )
            if self.link_page.canonical_page:
            ...
            try:
                return TranslatablePage.objects.get(
                    language=language, canonical_page=can_page
                )
...
class CustomComment(XtdComment):
    page = ParentalKey(
        ArticlePage, on_delete=models.CASCADE, related_name="customcomments"
    )
